...
import { mockUsers, weeklyData } from '@/lib/mock-data'
...
export default function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Generate mock proof images
  const mockProofImages = Array.from({ length: 12 }, (_, i) => ({
    id: `proof-${i}`,
        ...
        <div>
          ...
          <div className="grid grid-cols-3 gap-2">
            {mockProofImages.map((proof) => (
              ...
              >
                <span className="text-xs font-mono">{proof.duration.toFixed(1)}h</span>
                <span className="text-[10px] mt-1">
                  {proof.date.toLocaleDateString('en-US', { month: 'short', day: 'numeric' })}
                </span>
              </div>
            ))}
          </div>
